...
function GM:ProcessResultText( result, resulttext )
	...
	// for a free for all you could do... if type(result) == "Player" and IsValid( result ) then return result:Name().." - победитель" or whatever
...
function GM:RoundEnd()
	...
	if ( !GAMEMODE:InRound() ) then 
		...
		MsgN("ВНИМАНИЕ: RoundEnd вызывается, когда режим игры не в раунде...")
		debug.Trace()
		return 
	end
...
function GM:CheckPlayerDeathRoundEnd()
	...
	if ( GAMEMODE.RoundEndsWhenOneTeamAlive ) then
		...
		if ( table.Count( Teams ) == 0 ) then
			...
			GAMEMODE:RoundEndWithResult( 1001, "Ничья, все проиграли!" )
			return
...
function GM:CheckRoundEnd()
	...
	// if something then call GAMEMODE:RoundEndWithResult( TEAM_BLUE, "Синяя команда съела все грибы!" )
