...
function GM:VoteForChange( ply )
	...
	local NeedTxt = "" 
	if ( VotesNeeded > 0 ) then NeedTxt = ", Color( 80, 255, 50 ), [[ (нужно на "..VotesNeeded.." больше) ]] " end
	
	if ( CurTime() < GetConVarNumber( "fretta_votegraceperiod" ) ) then -- can't vote too early on
		local timediff = math.Round( GetConVarNumber( "fretta_votegraceperiod" ) - CurTime() );
		BroadcastLua( "chat.AddText( Entity("..ply:EntIndex().."), Color( 255, 255, 255 ), [[ проголосовал чтобы поменять режим]] )" )
	else
		BroadcastLua( "chat.AddText( Entity("..ply:EntIndex().."), Color( 255, 255, 255 ), [[ проголосовал чтобы поменять режим]] "..NeedTxt.." )" )
	end
	
	Msg( ply:Nick() .. " проголосовал чтобы поменять режим\n" )
...
function GM:FinishMapVote()
	...
	if self.WinningMap then
		...
		ErrorNoHalt("Нет карт для этого режима, принудительное использование карты gm_construct\nПожалуйста, измените это как можно скорее!\n")
