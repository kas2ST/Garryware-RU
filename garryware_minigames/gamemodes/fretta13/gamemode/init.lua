...
function GM:PlayerRequestClass( ply, class, disablemessage )
	...
	if ( ply:Alive() && SERVER ) then
		...
		if ( !disablemessage ) then
			ply:ChatPrint( "Твой класс измениться на '".. player_class.GetClassName( RequestedClass ) .. "' когда ты возродишься" )
		end
...
function GM:OnPlayerChangedTeam( ply, oldteam, newteam )
	...
	--PrintMessage( HUD_PRINTTALK, Format( "%s присоединился к '%s'", ply:Nick(), team.GetName( newteam ) ) )
		...
		if ( id ~= highest and id > 0 && id < 1000 && team.Joinable( id ) ) then
			if team.NumPlayers( id ) < team.NumPlayers( highest ) then
				while team.NumPlayers( id ) < team.NumPlayers( highest ) - 1 do
					...
					PrintMessage(HUD_PRINTTALK, ply:Name().." было изменено на "..team.GetName( id ).." для командного баланса. ("..reason..")" )
...
function GM:FindLeastCommittedPlayerOnTeam( teamid )
	...
	if worstteamswapper then
		return worstteamswapper, "Недавно они сменили команду"
	end

	return worst, "Наименьшее количество очков в команде"
...
function GM:EndOfGame( bGamemodeVote )
	...
	if ( bGamemodeVote ) then
		...
		MsgN( "Начинается голосование за режим..." )
		PrintMessage( HUD_PRINTTALK, "Начинается голосование за режим..." );
