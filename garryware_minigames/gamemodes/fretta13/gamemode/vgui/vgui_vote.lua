...
function PANEL:ChooseGamemode()
	...
	self.lblActionName:SetText( "Какой Режим Следующий?" )
	self.ctrlList:Clear()
...
function PANEL:ChooseMap( gamemode )
	...
	self.lblActionName:SetText( "Какая Карта?" )
	self:ResetPeeps()
	self.ctrlList:Clear()
	
	local gm = g_PlayableGamemodes[ gamemode ]
	if ( !gm ) then MsgN( "ИГРОВОЙ РЕЖИМ ОТСУТСТВУЕТ, НЕЛЬЗЯ ПРОГОЛОСОВАТЬ ЗА КАРТУ ", gamemode ) return end	
