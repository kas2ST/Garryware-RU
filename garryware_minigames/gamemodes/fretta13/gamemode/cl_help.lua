...
function GM:ShowHelp()
	...
	if ( !IsValid( Help ) ) then
		...
		Help:SetHeaderText( GAMEMODE.Name or "Безымянный Режим" )
		Help:SetHoverText( GAMEMODE.Help or "Помощь Недоступна" );
		
		Help.lblFooterText.Think = function( panel ) 
										local tl = GAMEMODE:GetGameTimeLeft()
										if ( tl == -1 ) then return end
										if( GetGlobalBool( "IsEndOfGame", false ) ) then panel:SetText( "Игра Закончилась..." ) return end
										if( GAMEMODE.RoundBased && CurTime() > GAMEMODE:GetTimeLimit() ) then panel:SetText( "Игра закончится после этого раунда" ) return end
										
										panel:SetText( "Оставшееся Время: " .. util.ToMinutesSeconds( tl ) ) 
									end

		if ( GetConVarNumber( "fretta_voting" ) != 0 ) then
			local btn = Help:AddSelectButton( "Голосуйте, Чтобы Поменять", function() RunConsoleCommand( "voteforchange" ) end )
			btn.m_colBackground = Color( 255, 200, 100 )
			btn:SetDisabled( LocalPlayer():GetNWBool( "WantsVote" ) ) 
		end
		
		if ( GAMEMODE.TeamBased ) then
			local btn = Help:AddSelectButton( "Изменить Команду", function() GAMEMODE:ShowTeam() end )
		...
		if ( !GAMEMODE.TeamBased && GAMEMODE.AllowSpectating ) then
			...
			if ( LocalPlayer():Team() == TEAM_SPECTATOR ) then
				...
				local btn = Help:AddSelectButton( "Присоединиться", function() RunConsoleCommand( "changeteam", TEAM_UNASSIGNED ) end )
				btn.m_colBackground = Color( 120, 255, 100 )
		...
		if ( IsValid( LocalPlayer() ) ) then
			...
			if ( Classes && #Classes > 1 ) then
				local btn = Help:AddSelectButton( "Изменить Класс", function() GAMEMODE:ShowClassChooser( LocalPlayer():Team() ) end )
				btn.m_colBackground = Color( 120, 255, 100 )
			end
		...
		if ( GAMEMODE.SelectModel ) then
			...
			Help:AddPanelButton( "icon16/user.png", "Выбери Игровую Модельку", CreateModelPanel )
		...
		if ( GAMEMODE.SelectColor ) then
			...
			Help:AddPanelButton( "icon16/application_view_tile.png", "Выбери Игровой Цвет", CreateColorPanel )
