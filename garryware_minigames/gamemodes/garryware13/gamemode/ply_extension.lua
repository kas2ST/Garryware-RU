...
function meta:PrintComboMessagesAndEffects( compareCombo )
	if (compareCombo == GAMEMODE:GetBestStreak()) then
		GAMEMODE:PrintInfoMessage( self:GetName(), " сравнял счёт с ", "Лучшей Серией Побед на сервере из " .. compareCombo .. " Вейров!" )
	...
	elseif (compareCombo >= 3) and (compareCombo == self:GetBestCombo()) then 
		GAMEMODE:PrintInfoMessage( self:GetName(), " забил свой ", "Собственную Лучшую Серию Побед из " .. compareCombo .. " Вейров!" )
	...
	elseif (compareCombo >= 3) then 
		GAMEMODE:PrintInfoMessage( self:GetName(), " набрал ", "Серию Побед из " .. compareCombo .. " Вейров." )
