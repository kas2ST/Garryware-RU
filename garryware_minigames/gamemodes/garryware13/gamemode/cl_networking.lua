...
netstream.Hook("Transit", function(data)
	if data then
		...
		TransitVGUI:SetSubtitle("Фейл-о-метр сервера : " .. tostring( 100 - theoWinFailNum ) .. "% ... " .. EvaluateFailure( theoWinFailNum ) .. "!"  )
...
netstream.Hook("gw_yourstatus", function(data)
	...
	if !isServerGlobal then
		sText = ((yourStatus and "Успех!") or "Провал!") -- MaxOfS2D you fail
		if yourStatus then
		...
		sText = ((yourStatus and "Все выиграли!") or "Все проиграли!")
