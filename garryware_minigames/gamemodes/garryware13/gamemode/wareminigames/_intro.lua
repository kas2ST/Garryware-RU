...
function WARE:Initialize()
	...
	GAMEMODE:DrawInstructions( "Новая GarryWare игра начинается!" )
...
function WARE:StartAction()
	GAMEMODE:DrawInstructions( "Правила просты: делайте то, что вам говорят!" )
...
function WARE:PhaseSignal( iPhase )
	if GAMEMODE:GetCurrentPhase() == 2 then
		GAMEMODE:DrawInstructions( "Чтобы забраться на коробку, подпрыгните, а затем нажмите Ctrl, находясь в воздухе!" , self.RoleColor )
		
	end
	
	if GAMEMODE:GetCurrentPhase() == 3 then
		GAMEMODE:DrawInstructions( "Попробуйте забраться на ящик!" , self.RoleColor )
...
function WARE:EndAction()
	GAMEMODE:DrawInstructions( "Игра начинается! Повеселитесь!" )
