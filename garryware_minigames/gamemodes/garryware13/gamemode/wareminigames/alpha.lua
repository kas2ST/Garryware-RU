...
function WARE:Initialize()
	...
	--GAMEMODE:DrawInstructions("Стреляйте в " .. self.NumberSpawns .." ящиков в правильном порядке!" )
	GAMEMODE:DrawInstructions("Стреляй в правильном порядке!" )
	
	self.Crates = {}
	self.Numbers = {}
	self.Sequence = {}
	
	local leftchars = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ"
	local tableseq = {}
...
function WARE:StartAction()
	if (self.croissant == 1) then
		...
		GAMEMODE:DrawInstructions( "В алфавитном порядке! (A , B , C...)" )
		GAMEMODE:PrintInfoMessage( "Последовательность порядка", " - ", "алфавитный (A , B , C...)!" )
	else
		for k=self.NumberSpawns,1,-1 do
			table.insert( self.Sequence , k )
		end
		GAMEMODE:DrawInstructions( "В обратном порядке! (C , B , A...)" )
		GAMEMODE:PrintInfoMessage( "Последовательность порядка", " - ", "обратный (C , B , A...)!" )
	end
...
function WARE:EndAction()
	...
	GAMEMODE:PrintInfoMessage( "Последовательность", " была ", message .."!" )
	GAMEMODE:DrawInstructions( "Последовательность была ".. message .."!" , self.CorrectColor)
end
