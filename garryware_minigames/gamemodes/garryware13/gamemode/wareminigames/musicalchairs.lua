...
function WARE:PhaseSignal( iPhase )
	if iPhase == 2 then
		GAMEMODE:DrawInstructions( "Сейчас сядь в капсулу!", self.RoleColor )
...
function WARE:CanPlayerEnterVehicle( ply, vehEnt, role )
	...
	if !ply:IsWarePlayer() then
		if !ply.__ISEXPLOITER then
			GAMEMODE:PrintInfoMessage( ply:Name(), " (SteamID : " .. ply:SteamID() .. " ) ", "теперь зарегистрирован как эксплуататор ошибок." )
