...
function PANEL:Think()
	if self.m_player == nil then
		print("ОШИБКА разработчика: Метка игрока думала, будучи NIL! Удалено.")
		self:Remove()
		return
	end
	...
	if self.m_player:IsWarePlayer() then
		...
		if self.m_player:Team() == TEAM_SPECTATOR then
			self:SetText( self.m_player:Name() .. " (НАБЛ.)" )
			
		elseif self.m_player:Team() == TEAM_UNASSIGNED then
			self:SetText( self.m_player:Name() .. " (ЛОББИ)" )
